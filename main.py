...
class DetectWorker():
    ...
    def run(self):
        if self.type_ == 1:
            ...
            self.model2 = YOLO(".\\__pycache__\\best2.pt")
            self.model3 = YOLO(".\\__pycache__\\best3.pt")
 
            #Clear temp 
            self.clear_filefolder(".\\__pycache__\\md1\\cropped_results")
            self.clear_filefolder(".\\__pycache__\\md1")
            self.clear_filefolder(".\\__pycache__\\md2\\detect_results")
            self.clear_filefolder(".\\__pycache__\\md3\\detected_results\\crops")
            #run processes
            self.detect_cccd()
            self.detect_corners()
            self.detect_lines()
    ...
    def detect_lines(self):
        ...
        import os
        import math
        os.makedirs(".\\__pycache__\\md3\\detected_results", exist_ok=True)
        os.makedirs(".\\__pycache__\\md3\\detected_results\\crops", exist_ok=True)

        folder = ".\\__pycache__\\md2\\detect_results"
        img_files = [
        ...
        for i, img_path in enumerate(img_files):
            ...
            for j, polygon in enumerate(r.masks.xy):
                ...
                crop_name = f".\\__pycache__\\md3\\detected_results\\crops\\{file_name.replace('.jpg','')}-{class_name}.jpg"
                cv2.imwrite(crop_name, crop)
            boxed_name = f".\\__pycache__\\md3\\detected_results\\boxed_{file_name}"
        ...
        config['weights'] = '.\\__pycache__\\vgg_transformer.pth'
        config['cnn']['pretrained'] = False
        #config['device'] = 'cpu'
        config['device'] = 'cpu'
        detector = Predictor(config)

        crop_folder = '.\\__pycache__\\md3\\detected_results\\crops'
        goc_folder = '.\\__pycache__\\md2\\detect_results'
        results_folder = ".\\__pycache__\\temp_rs"
        os.makedirs(results_folder, exist_ok=True)

        ocr_result_file = '.\\__pycache__\\md4\\ocr_results.txt'
        ocr_data = []
        ...
        for goc_name, infos in goc_dict.items():
            ...
            font_path = ".\\__pycache__\\arial.ttf"  # Bạn thay đường dẫn đúng của bạn!
    ...
    def detect_corners(self):
        os.makedirs(".\\__pycache__\\md2\\detect_results", exist_ok=True)
        folder = ".\\__pycache__\\md1\\cropped_results"
        img_files = [
            os.path.join(folder, f)
            for f in os.listdir(folder)
        ...
        for i, img_path in enumerate(img_files):
            ...
            if 'quoc_huy' in centers and 'qr' in centers:
                ...
                cv2.imwrite(f".\\__pycache__\\md2\\detect_results\\{file_name}.jpg", img)  # Lưu nguyên gốc
                continue
            ...
            cv2.imwrite(f".\\__pycache__\\md2\\detect_results\\{file_name}.jpg", rotated)
